...
export function PredictionHistory() {
  ...
  const getPredictionColor = (pred: string) => {
    switch (pred) {
      case "BUY": return "text-success border-success/20 bg-success/10";
      case "SELL": return "text-destructive border-destructive/20 bg-destructive/10";
  ...
  const calculateSuccessRate = () => {
    ...
    const totalConfidence = history.reduce((sum, pred) => sum + pred.confidence, 0);
      ...
      {isVisible && (
        <CardContent>
          <div className="space-y-3 max-h-60 overflow-y-auto">
            {history.slice().reverse().map((pred) => (
              <div 
                key={pred.id}
              ...
              >
                <div className="flex items-center space-x-3">
                  <Badge 
                    variant="outline" 
                    className={cn("flex items-center space-x-1", getPredictionColor(pred.prediction))}
                  >
